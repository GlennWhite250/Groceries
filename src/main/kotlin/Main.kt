fun main(args: Array<String>) {
    ...
    val lowestQuantity = groceries.minBy {
        it.quantity
    }
    ...
    val groceryNames = groceries.map {
        it.name
    }
    println("groceryNames: $groceryNames.")
    val halfUnitPrice = groceries.map {
        it.unitPrice * 0.5
    }
    ...
    val productOfInts = ints.fold(1){ runningProduct, item ->
        runningProduct * item
    }
    println("productOfInts: $productOfInts.")

    val name = groceries.fold(" "){ string, item ->
        string + " ${item.name}."
    }
    println("names: $name")
